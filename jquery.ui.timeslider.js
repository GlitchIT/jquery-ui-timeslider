(function($) {
	$.widget("ui.timeslider", {
		...
		_slideTime: function(event, ui) {
			...
			//Routine for range slider
			if (that.slider("option", "range")) {

				//Changed object from which textbox values are called from. See stackoverflow.com/q/11418610
				//This if...then... block checks to see if the interactive element 'ui' has been created, and if so, use it to read values
				...
				//Routine for non-range slider
			} else {

				//Chnaged object from which textbox values are called from. See stackoverflow.com/q/11418610
				//This if...then... block checks to see if the interactive element 'ui' has been created, and if so, use it to read values
				if (typeof ui == 'undefined') {
					startTime = that.timeslider('getTime', that.slider("value"), that.timeslider('option', 'clockFormat'));
				} else {
					startTime = that.timeslider('getTime', ui.value, that.timeslider('option', 'clockFormat'));
				}
